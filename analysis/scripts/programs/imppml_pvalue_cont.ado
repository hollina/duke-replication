/**** Script: imppml_pvalue_cont.ado 
	* Purpose: 
	*	- Runs ppmlhdfe regressions
*/

cap program drop imppml_pvalue_cont
program define imppml_pvalue_cont
	
	syntax, y_stub(string) suffix(string) Treated(string) Absorb(string) [pooled controls(string) wgt(string) restrict(string) stacked(string) COLumn(string) save_path(string) save_prefix(string)]
	...
	if "`column'" != "" {
		...
		if "`save_path" != "" {
			...
			local estsave_interact		"est save `save_path'/`save_prefix'p4_c`column', replace"
		}
	...
	* none
	...
	local scaled_coef "exp(_b[`treated'])-1"
	local comtype "nlcom"
	...
	if "`pooled'" != "" {
		...
		preserve
			...
			local scaled_coef "exp(_b[c.`treated'])-1"
			local int_scaled_coef "exp(_b[c.`treated'#2.race])-1"
			local comtype "nlcom"
						
			// Fully interacted - Racial IMR gap
			
			`eststo_prefix_interact' ppmlhdfe `depvar_interact' c.`treated'##i.race `reg_wgt_interact', absorb(`absorb' `interacted_controls') vce(cluster fips)
			di "`e(cmdline)'"
			lincomestadd2a 100*(`scaled_coef'), comtype(`comtype') statname("pct_efct_")
			lincomestadd2a 100*(`int_scaled_coef'), comtype(`comtype') statname("int_pct_efct_")
			test _b[c.`treated'#2.race] = 0

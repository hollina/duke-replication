/**** Script: imppml_regsave.ado 
	* Purpose: 
	*	- Runs ppmlhdfe regressions
*/

cap program drop imppml_regsave
program define imppml_regsave
	
	syntax, y_stub(string) suffix(string) Treated(string) Absorb(string) [pooled controls(string) wgt(string) restrict(string) stacked(string) COLumn(string) reg_filename(string) replace_option(string)]
	...
	* none
	...
	if "`pooled'" != "" {
		...
		regsave 1.treated using "$PROJ_PATH/analysis/processed/temp/`reg_filename'.dta", t p ci autoid addlabel(rep,`i',b,`e(pct_efct_b_num)',se,`e(pct_efct_se_num)',t,`e(pct_efct_t_num)') `replace_option'
	}
		...
		regsave 1.treated using "$PROJ_PATH/analysis/processed/temp/`reg_filename'_bk.dta", t p ci autoid addlabel(rep,`i',b,`e(pct_efct_b_num)',se,`e(pct_efct_se_num)',t,`e(pct_efct_t_num)') `replace_option'
		...
		regsave 1.treated using "$PROJ_PATH/analysis/processed/temp/`reg_filename'_wt.dta", t p ci autoid addlabel(rep,`i',b,`e(pct_efct_b_num)',se,`e(pct_efct_se_num)',t,`e(pct_efct_t_num)') `replace_option'
		...
		preserve
			...
			local scaled_coef "exp(_b[1.`treated'])-1"
			local int_scaled_coef "exp(_b[1.`treated'#2.race])-1"
			local comtype "nlcom"
						
			// Fully interacted - Racial IMR gap
			
			`eststo_prefix_interact' ppmlhdfe `depvar_interact' b0.`treated'##i.race `reg_wgt_interact', absorb(`absorb' `interacted_controls') vce(cluster fips)
			di "`e(cmdline)'"
			lincomestadd2a 100*(`scaled_coef'), comtype(`comtype') statname("pct_efct_")
